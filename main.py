...
@cli.command()
@click.option('--config', type=click.Choice(['base', 'pdr', 'max', 'max_no_ser_tab', 'max_nst_o3m', 'max_st_o3m', 'gemini_thinking']), default='base', help='Configuration preset to use')
...
@cli.command()
@click.option('--host', default='0.0.0.0', help='Host to run the web UI on')
@click.option('--port', default=5000, help='Port to run the web UI on')
@click.option('--debug', is_flag=True, help='Run in debug mode')
def webui(host, port, debug):
    """Launch the Web UI for interactive RAG queries."""
